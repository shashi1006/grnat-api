...
func (r *scoringRepo) ListTopGrantsForOrg(ctx context.Context, orgID uuid.UUID, limit, offset int32) ([]*repository.ScoredGrant, error) {
	const q = `
		SELECT cs.id, cs.org_id, cs.grant_id, cs.total_score, cs.tier, cs.dimension_scores,
		       ...
		       g.focus_areas, g.eligible_org_types, g.min_award_amount, g.max_award_amount,
		       g.total_funding_available, g.application_url,
	...
	for rows.Next() {
		...
		score, err := scanScoreWithExtra(rows,
			...
			&sg.FocusAreas, &sg.EligibleOrgTypes, &sg.MinAwardAmount, &sg.MaxAwardAmount,
			&sg.TotalFundingAvailable, &sg.ApplicationURL,
